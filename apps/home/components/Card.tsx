...
function Card({blog,isSmall,index}:{blog:IBlog,isSmall?:boolean,index?:number}) { 
    ...
    try
    {
      if(doesUserSaveBlog.data){
        if (doesUserSaveBlog.data.doesSave){
          ...
          axios.delete("/blog/save",{withCredentials:true})
          .then((res) => {
          ...
          axios.post("/blog/save",{ blogId:blog.id},{withCredentials:true})
          .then((res) => {
            mutate(`/blog/isSaved/${blog.id}`)
          })
          .catch((err) => console.log("ERROR"))
  ...
  return (
    <div>
      <BlogCard props={blog} session={session.session} likeclick={() => handleLike()} doesLike={doesUserLikeBlog.data?.doesLike} isSmall={isSmall} index={index} bookmarkclick={handleBookmark} doesSave={doesUserSaveBlog.data?.doesSave}/>
    </div>
  )
}
